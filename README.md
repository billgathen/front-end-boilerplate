...
- The code you work on is in the ```src``` directory structure: HTML goes in ```html```, JavaScript goes in the ```scripts``` directory, CSS and Sass files go in ```styles```, images go in ```images```. It will all be optimized and copied into the ```dist``` directory as you develop.
